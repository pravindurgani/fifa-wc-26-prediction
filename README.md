...
- **Final scores**: auto-fed via API-Football (active in production — Vercel Cron dispatches the GitHub Actions orchestrator every 10 min during the tournament window; completed matches are locked and the simulator re-runs. Typical refresh lands ~30–90 s after a final whistle, bounded by API-Football's FT-status propagation. GitHub's native cron remains configured as a redundant secondary path).
...
- **Live scores**: **API-Football (active in production)** — Vercel Cron (`/api/dispatch`) fires every 10 min during the tournament window (11 Jun → 19 Jul 2026) and dispatches a GitHub Actions workflow; completed matches are locked, the simulator re-runs, and idempotent commit-on-change avoids no-op deploys. Typical refresh lands ~30–90 s after a final whistle, bounded by API-Football's FT-status propagation.
