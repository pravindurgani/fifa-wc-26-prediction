...
- **Final scores**: auto-fed via API-Football (active in production — scheduled every 10 min during the tournament window, locks completed matches, re-runs the simulator; GitHub may throttle scheduled events on free-tier runners, so real refresh sits between ~10 min and ~1 h after a final whistle).
...
- **Live scores**: **API-Football (active in production)** — scheduled every 10 min during the tournament window (11 Jun → 19 Jul 2026); completed matches are locked and the simulator re-runs. GitHub Actions may throttle the cron under load — typical refresh sits between ~10 min and ~1 h after a final whistle.
